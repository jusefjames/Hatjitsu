...
Room.prototype.enter = function(socket, data) {
  ...
  if (this.connections[data.sessionId]) {
    ...
    this.connections[data.sessionId] = { sessionId: data.sessionId, socketId: socket.id, vote: null, voter: true, voterName: 'Guest' };
  }
}
...
Room.prototype.changeName = function(data) {
  // console.log("changeName data: " + data);
  if (this.connections[data.sessionId]) {
    this.connections[data.sessionId]['voterName'] = data.voterName;
    if (!data.voterName) {
      this.connections[data.sessionId]['voterName'] = null;
    }
    // console.log("name set to " + data.voterName + " for " + data.sessionId);
  }
  this.io.sockets.in(this.roomUrl).emit('voter name changed');
}
